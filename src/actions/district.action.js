...
export const fetchDistrictsCall =
  (pageDetails, successCallback, failureCallback) => async (dispatch) => {
    ...
    try {
      ...
      get(data, 'content', []).forEach((item) => modifyDistrictItem(item))
...
export const fetchDistrictsByPublicIdsCall =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      data.forEach((item) => modifyDistrictItem(item))
      if (typeof successCallback === 'function') {
        successCallback(data)
      }
    } catch (error) {
...
export const fetchDistrictByIdCall = (id, successCallback, failureCallback) => async (dispatch) => {
  dispatch(toggleLoaderAction(true))
  try {
    const url = `${API.CORE.DISTRICT}/${id}`
    const { data } = await fetch(url)
    modifyDistrictItem(data)
    if (typeof successCallback === 'function') {
      successCallback(data)
    }
  } catch (error) {
...
export const addDistrictCall = (payload, successCallback, failureCallback) => async (dispatch) => {
  dispatch(toggleLoaderAction(true))
  try {
...
export const uploadDistrictLogoCall =
...
const modifyDistrictItem = function (item) {
  if (isEmpty(item)) {
    return
  }
  item.id = item.dst_id
