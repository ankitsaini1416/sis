...
import DailyReportsSkeleton from './DailyReportsSkeleton'
...
function DailyReport({
  ...
  const exportExcel = () => {
    const mappedRequests = dailyReportsList
    ...
    exportToCSV(mappedRequests, t('dailyReport'))
  }
  ...
  const enrollmentStatus = (row) => {
    ...
    return (
      <Box
        component="span"
        className={enrolledMap[row?.enrStatus].style}
        id={row.enrId}
        tabIndex={0}
      >
    ...
    <>
      ...
      <Paper rounded={true} elevation={1} className="paper-round">
              ...
              >
                <Grid spacing={2} container className="filter-search-section">
                  ...
                  <Grid item xs={12} sm={6} md={6} lg={4} xl={3}>
                    ...
                    >
                      {districts.map((topic) => (
                        <MenuItem key={topic.dst_id} value={topic.dst_id}>
                          {topic.dst_name}
                        </MenuItem>
                      ))}
                    </TextField>
        ...
        <CustomTable
          ...
          selected={checkState}
          setSelected={onCheck}
          isSelection={dailyReportsList.length > 0 ? true : false}
        >
          {dailyReportsList.map((row) => {
            return (
              <TableRow hover data-id={row.id} key={row.id}>
                ...
                {initialHeadcells.includes('address') && (
          ...
          >
            ...
            <Box mt={{ xs: 1, sm: 0 }}>
              <Typography component="p" variant="body2" tabIndex={0}>
                Showing {dailyReportsList.length} rows out of {pageDetails.total}
              </Typography>
            </Box>
          </Box>
        )}
...
DailyReport.propTypes = {
  ...
  dailyReportsList: PropTypes.array,
