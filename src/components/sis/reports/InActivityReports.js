...
import { inactivityDurations, ragistrationStatus } from '../../../helpers/stub'
import {
  enrolledMap,
  exportToCSV,
  getFullName,
...
// import InactivityReportsSkeleton from './InactivityReportsSkeleton'
import useStyles from './Reports.Style'

function InActivityReports({
  initialHeadcells,
  allHeadCells,
  setHeadcells,
  order,
  ...
  const breadcrumbData = [
    ...
    {
      title: t('reports'),
      href: ROUTES.INACTIVITYREPORTS,
    },
    {
      title: t('inActivityReports'),
      href: '',
    },
  ]
  ...
  const exportExcel = () => {
    const mappedRequests = inactivityReportsList
    ...
    exportToCSV(mappedRequests, t('inActivityReports'))
  }
  ...
  // if (isEmpty(inactivityReportsList)) {
  //   return <InactivityReportsSkeleton />
  // }
  return (
    <>
      <Box py={2}>
      ...
      <Paper rounded={true} elevation={1} className="paper-round">
                  ...
                  <Grid item xs={12} sm={6} md={6} lg={4} xl={4}>
                    <Box className="filter-surface-field">
                      ...
                      >
                        ...
                        {masterData.gender?.map((options) => (
                          <ToggleButton key={options} value={options}>
                            {options}
                          </ToggleButton>
                        ))}
                      </CustomToggleGroup>
                    ...
                    >
                      <Box mr={1}>
                        <Button
                          ...
                          disabled={
                            !(filter.districtId && filter.schoolId && filter.inactivityDuration)
                          }
                        >
                          {t('generateReport')}
                        </Button>
        ...
        <CustomTable
          ...
          data={inactivityReportsList}
          headCells={allHeadCells.filter((cell) => initialHeadcells.includes(cell.id))}
          dataParameter={dataParameter}
          selected={checkState}
          setSelected={onCheck}
          isSelection={inactivityReportsList.length > 0 ? true : false}
        >
          {inactivityReportsList.map((row) => {
            return (
              <TableRow hover data-id={row.id} key={row.id}>
                ...
                {initialHeadcells.includes('last_login') && (
                  <TableCell tabIndex={0}>{row.lastLoginTime}</TableCell>
                )}
          ...
          >
            ...
            <Box mt={{ xs: 1, sm: 0 }}>
              <Typography component="p" variant="body2" tabIndex={0}>
                Showing {inactivityReportsList.length} rows out of {pageDetails.total}
              </Typography>
            </Box>
          </Box>
        )}
...
InActivityReports.propTypes = {
  ...
  inactivityReportsList: PropTypes.array,
  masterData: PropTypes.object,
}

InActivityReports.defaultProps = {
  initialHeadcells: [],
  allHeadCells: [],
  setHeadcells: () => {},
  order: '',
...
export default InActivityReports
