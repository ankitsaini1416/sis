...
function InstructerGenericReport({
  order,
  orderBy,
  setOrder,
  setOrderBy,
  ...
  const exportExcel = () => {
    const mappedRequests = 'studentReportList'
    ...
    exportToCSV(mappedRequests, 'Instructor Reports')
  }
  ...
  return (
    <>
                  ...
                  <TextField
                    ...
                    InputProps={{
                      endAdornment: (
                        <InputAdornment position="end">
                          <Tooltip title={t('fields:searchInstructorHelp')} placement="top">
                    ...
                    label={
                      <span style={visuallyHidden}>
                        ({t('fields:instructor')}) ({t('fields:searchInstructorHelp')})
                      </span>
                    }
                  />
                </Grid>
      ...
      <CustomTable
        ...
        data={instructerReportList}
        headCells={allHeadCells.filter((cell) => initialHeadcells.includes(cell.id))}
        dataParameter={dataParameter}
        selected={checkState}
        setSelected={onCheck}
        isSelection={instructerReportList.length > 0 ? true : false}
      >
        {instructerReportList.map((row) => {
          return (
            <TableRow hover data-id={row.id} key={row.id}>
              ...
              {initialHeadcells.includes('email_Id') && (
                <TableCell tabIndex={0}>
                  <Tooltip placement="top" title={row.email_Id}>
                    <Box width="150px" className="text-ellipsis">
                      {row.email_Id}
                    </Box>
                  </Tooltip>
                </TableCell>
              ...
              {initialHeadcells.includes('course_id') && (
                <TableCell tabIndex={0}>{row.course_id}</TableCell>
              )}
              {initialHeadcells.includes('course_group') && (
                <TableCell tabIndex={0}>{row.course_group}</TableCell>
              )}
              {initialHeadcells.includes('course_Name') && (
                <TableCell tabIndex={0}>{row.course_Name}</TableCell>
              )}
        ...
        >
          ...
          <Box mt={{ xs: 1, sm: 0 }}>
            <Typography component="p" variant="body2">
              Showing {instructerReportList.length} rows out of {pageDetails.total}
            </Typography>
          </Box>
        </Box>
      )}
...
InstructerGenericReport.propTypes = {
...
InstructerGenericReport.defaultProps = {
  order: '',
  orderBy: '',
  setOrder: () => {},
  setOrderBy: () => {},
...
export default InstructerGenericReport
